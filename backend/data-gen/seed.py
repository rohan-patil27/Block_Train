...
def seed_data(db_session=None):
    ...
    for i in range(5):
        ...
        if not train:
            train = Train(train_number=t_num, train_type=random.choice(["Express", "Freight", "Passenger"]))
            db.add(train)
        trains.append(train)
    db.commit()

    now = datetime.utcnow()
    # Add some schedules to create conflicts and shadow block opportunities
    # Train 0: scheduled 10 to 40 mins from now at section 1
    ts1 = TrainSchedule(
    ...
    # Train 1: scheduled 60 to 90 mins from now at section 1
    ts2 = TrainSchedule(
        train_id=trains[1].id,
        node_id=nodes[0].id,
        arrival_time=now + timedelta(minutes=60),
        departure_time=now + timedelta(minutes=90),
        sequence=1
    )
    db.add(ts1)
    db.add(ts2)
